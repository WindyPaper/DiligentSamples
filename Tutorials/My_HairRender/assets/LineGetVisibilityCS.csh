...
void AddToVisibilityBuffer(uint line_idx, float px, float py, float max_z, float voxel_z_offset)
{
...
void CSMain(uint3 id : SV_DispatchThreadID, uint3 group_id : SV_GroupID, uint group_idx : SV_GroupIndex)
{
    ...
    if(steep)
    {
        for(int i = s_x; i <= e_x; ++i)
        {
            ...
            if(bright > 0.0f)
            {
                AddToVisibilityBuffer(w_x, w_y, LineBBoxMax.z, VoxelZOffset);
            }
